...
/**
 * @brief store triangle's vertex,color,normal and texcoord
 */
class Triangle
{
  public:
    ...
    const std::array<glm::vec4, 3> &getVertices() const
    {
        return vertex;
    }
    ...
    const glm::vec3 &getNormal(uint8_t index) const
    {
        return normal[index];
    }

    const std::array<glm::vec3, 3> &getNormals() const
    {
        return normal;
    }
