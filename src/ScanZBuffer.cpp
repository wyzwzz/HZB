...
#include <algorithm>
#include <iostream>
ScanZBuffer::ScanZBuffer(uint32_t w, uint32_t h) : w(w), h(h), active_polygon_table(nullptr), active_edge_table(nullptr)
{
    init();
}
...
template <class T> void traverseTable(T t)
{
    int cnt = 0;
    while (t != nullptr)
    {
...
void ScanZBuffer::scaningRaster()
{
    ...
    for (int cur_h = this->h - 1; cur_h >= 0; cur_h--)
    {
        ...
        while (p_act_edge != nullptr)
        {
            ...
            p_act_edge->xl +=
                std::fabs(p_act_edge->dxl) > 1.f ? p_act_edge->dxl / std::fabs(p_act_edge->dxl) : p_act_edge->dxl;
            p_act_edge->xr +=
                std::fabs(p_act_edge->dxr) > 1.f ? p_act_edge->dxr / std::fabs(p_act_edge->dxr) : p_act_edge->dxr;
...
void ScanZBuffer::addActiveEdgeTable(ClassfiedPolygonTable *polygon, ClassfiedEdgeTable *left_edge,
                                     ClassfiedEdgeTable *right_edge, float y)
{
    add_cnt++;
