...
class Bound2{
public:
    Bound2(const glm::vec2& min_,const glm::vec2& max_ ):
    min_point(min_),max_point(max_)
    {};
    Bound2(){
    ...
    bool containing(const Bound2& bound) const {
        if(bound.max_point.x<=this->max_point.x
        && bound.max_point.y<=this->max_point.y
        && bound.min_point.x>=this->min_point.x
        && bound.min_point.y>=this->min_point.y)
            return true;
        else return false;
    }
    ...
    friend std::ostream& operator<<(std::ostream& os,const Bound2& b){
        ...
        <<"\nmax_point: "<<b.max_point.x<<" "<<b.max_point.y;
        return os;
    }
...
public:
    ...
    parent(nullptr),bound(Bound2(min_,max_)),z_depth(0.f)
    {};
